...
class HTMLSection {
    ...
    updateCartQuantity() {
        ...
        const currentCartQuantity = cartArray.reduce((accumulator, currentValue) => {
...
class ShoppingItem {
    constructor(title, desc, price, category, id, img, categoryTitle) {
        this.title = title;
        this.desc = desc;
        this.price = price;
    ...
    addToLS() {
        ...
        const itemExists = existingItems.some((item) => item.id === this.id);
        // Přidat do pole
        if (!itemExists) {
...
class ItemInCart {
    constructor(ti, price, category, id, img, q) {
        this.title = ti;
        this.price = price;
